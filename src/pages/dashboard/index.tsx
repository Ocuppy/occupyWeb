...
import TransactionsTable from "@/components/dashboard/dashboard/TransactionsTable";
import { useGetSupermarketProfileQuery } from "@/store/redux/services/profileSlice/profileApiSlice";
import { useGetUserSupermarketsQuery } from "@/store/redux/services/profileSlice/profileApiSlice";
import { getCredentials } from "@/store/redux/services/authSlice/authSlice";
import { useAppDispatch, useAppSelector } from "@/store/redux/hooks";
...
const Page: NextPageWithLayout = () => {
  // const [showMarketDet, setShowMarketDet] = useState(false);
  // const [hasAttemptedToCreateStore, setHasAttemptedToCreateStore] =
  //   useState(false);
  ...
  // // Fetch supermarkets data for the user
  // const {
  //   data: supermarkets = [] as any[],
  ...
  // if (!isAccountVerified && hasAttemptedToCreateStore) {
  ...
  //         <Button
  //           onClick={() => {
  //             setHasAttemptedToCreateStore(true);
  //             Router.push("/dashboard/store/add");
  //           }}
  //         >
  //           Add Store
  //         </Button>
  ...
  //       <div className="mx-auto mt-8 flex max-w-[900px] flex-col gap-8">
  //         <h1>No Available Supermarkets</h1>
  //       </div>
  //     </div>
  //   );
  // }
  ...
  useEffect(() => {
    // Check if store was just created
    const storeCreatedFlag = localStorage.getItem("storeCreated");
    if (storeCreatedFlag === "true") {
  ...
  // Render logic based on account status and supermarkets
  if (!hasCreatedStore) {
    return (
      <div className="h-full rounded-md bg-white px-4 py-6">
        <SpaceBetween>
  ...
  // If supermarkets exist but account is not verified, show AccountStatus
  if (hasCreatedStore) {
    return <AccountStatus />;
  }

  // Render dashboard for verified accounts with supermarkets
  // return !showMarketDet ? (
  //   <div className="h-full rounded-md bg-white px-4 py-6">
  //     <SpaceBetween>
  //       <p className="text-[20px] font-medium">Manage Store</p>
    ...
    <div className="h-full rounded-md bg-white px-4 py-6">
      {!showMarketDet ? (
        <>
          ...
          <div className="mx-auto mt-8 flex max-w-[900px] flex-col gap-8">
            <StoreItem store={store} onClickStore={toggleMarketDet} />
            <StoreItem store={store} onClickStore={toggleMarketDet} />
            <StoreItem store={store} onClickStore={toggleMarketDet} />
          </div>
        </>
