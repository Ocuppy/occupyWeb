...
const AddProduct = () => {
  const { savedFormValues, onSaveFormValues, currentStep, goNext } = useSteppedFormContext();
  ...
  const [step2FormField, setFormField] = useState<IFieldValue[][]>([defaultStep2Fields]);
        ...
        <div className="h-full grow overflow-auto hideScroll border-dashed border rounded-md bg-white px-6 py-6">
          <>
            {isFirstStep ? (
              ...
              <WorkTimeActivities addNewField={addNewField} step2FormField={step2FormField} />
            )}
          </>
        </div>
