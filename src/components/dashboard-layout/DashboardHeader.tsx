...
const SEARCH_VISIBLE_ROUTES = ["/orders", "/inventory"];

const DashboardHeader = () => {
  const pathname = usePathname();
  const { token, notificationPermissionStatus } = useFcmToken();
  const showSearchBar = SEARCH_VISIBLE_ROUTES.some((route) =>
  ...
  return (
    <header className="fixed right-0 z-10 flex w-screen items-center bg-white py-5 xl:w-full xl:pl-[270px] xl:pr-4">
      <Flex className="w-full justify-between px-4 xl:pl-10">
        ...
        <div className="absolute left-1/2 flex -translate-x-1/2 transform items-center space-x-2 xl:hidden">
          <Label htmlFor="online-mode-mobile" className="">
            Online
          </Label>
          <Switch
        ...
        <div
          className={`transition-all duration-300 ${showSearchBar ? "w-full max-w-[500px]" : "w-0"}`}
        >
          {showSearchBar && (
            <input
              type="search"
